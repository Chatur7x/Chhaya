...
class FileTransfer {
  ...
  double get progress => totalChunks > 0 ? completedChunks / totalChunks : 0;
}

enum TransferStatus { pending, transferring, completed, failed, paused }

class FileVaultService {
  static const String _filesBoxName = 'vault_files';
  static const String _foldersBoxName = 'vault_folders';
  static const String _versionsBoxName = 'vault_versions';
  static const String _transfersBoxName = 'vault_transfers';
  static const String _tagsBoxName = 'vault_tags';
  static const String _binBoxName = 'vault_bin';

  Box<String>? _filesBox;
  ...
  static FileVaultService? _instance;

  static Future<FileVaultService> getInstance() async {
    if (_instance == null) {
      _instance = FileVaultService._internal();
      await _instance!._initialize();
    }
    return _instance!;
  }

  FileVaultService._internal();

  Future<void> _initialize() async {
    _filesBox = await Hive.openBox(_filesBoxName);
    ...
    _binBox = await Hive.openBox(_binBoxName);

    final appDir = await getApplicationDocumentsDirectory();
    _vaultDir = Directory('${appDir.path}/vault');
    _versionsDir = Directory('${appDir.path}/vault_versions');
  ...
  Future<VaultFile> saveFile({
    ...
    final destPath = '${_vaultDir.path}/$id';

    var bytes = await sourceFile.readAsBytes();
    var checksum = _computeChecksum(bytes);

    if (encrypt && _encryptionKey != null) {
      bytes = _encryptData(bytes);
    }
  ...
  Future<VaultFolder> createFolder({
    ...
    required String ownerId,
    bool isShared = false,
    List<String>? sharedWithDeviceIds,
  }) async {
  ...
  Future<void> renameFolder(String folderId, String newName) async {
    ...
    final updated = VaultFolder(
      ...
      sharedWithDeviceIds: folder.sharedWithDeviceIds,
  ...
  Future<void> deleteFolder(String folderId) async {
    final children = getFolders(parentId: folderId);
    for (final child in children) {
      await deleteFolder(child.id);
    }

    final files = getFiles(folderId: folderId);
    ...
    final folderPath = Directory('${_vaultDir.path}/$folderId');
    if (await folderPath.exists()) {
      await folderPath.delete(recursive: true);
    }
  }

  List<VaultFile> getFiles(
      {String? folderId,
      String? searchQuery,
      FileType? type,
      List<String>? tags}) {
    ...
    for (final key in _filesBox?.keys ?? []) {
      ...
      try {
        ...
        if (tags != null && tags.isNotEmpty) {
          if (!tags.any((tag) => file.tags.contains(tag))) continue;
        }

        if (searchQuery != null && searchQuery.isNotEmpty) {
          final query = searchQuery.toLowerCase();
          if (!file.name.toLowerCase().contains(query)) continue;
          if (!file.tags.any((t) => t.toLowerCase().contains(query))) continue;
        }
    ...
    for (final key in _foldersBox?.keys ?? []) {
      ...
      try {
        ...
        if (parentId == null && folder.parentId == null) {
          folders.add(folder);
        } else if (parentId != null && folder.parentId == parentId) {
          folders.add(folder);
        }
      } catch (_) {}
    }
  ...
  Future<void> restoreFile(String fileId) async {
    ...
    await _filesBox?.put(fileId, json);
    await _binBox?.delete(fileId);
  }
  ...
  Future<void> permanentlyDeleteFile(String fileId) async {
    ...
    final versionKeys = _versionsBox?.keys
            .where((k) => k.toString().startsWith(fileId))
            .toList() ??
        [];
  ...
  List<VaultFile> getDeletedFiles() {
    ...
    for (final key in _binBox?.keys ?? []) {
      final json = _binBox?.get(key);
      if (json == null) continue;
  ...
  Future<void> emptyBin() async {
    final keys = _binBox?.keys.toList() ?? [];
    for (final key in keys) {
      await permanentlyDeleteFile(key.toString());
    }
  }
  ...
  Future<void> saveVersion(String fileId) async {
    final json = _filesBox?.get(fileId);
    if (json == null) return;
    ...
    final versionId = '${fileId}_v${file.version}';
    final versionPath = '${_versionsDir.path}/$versionId';

    await File(versionPath).writeAsBytes(bytes);
  ...
  List<FileVersion> getVersions(String fileId) {
    ...
    for (final key in _versionsBox?.keys ?? []) {
      final json = _versionsBox?.get(key);
      if (json == null) continue;
  ...
  Future<void> rollbackVersion(String fileId, int versionNumber) async {
    final versionId = '${fileId}_v$versionNumber';
    ...
    final versionBytes = await versionFile.readAsBytes();
    await currentLocalFile.writeAsBytes(versionBytes);
  }

  Map<String, int> getStorageUsage() {
  ...
  Future<void> shareFolder(String folderId, List<String> deviceIds) async {
    final json = _foldersBox?.get(folderId);
    if (json == null) return;
