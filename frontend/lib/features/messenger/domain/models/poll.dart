...
class Poll extends HiveObject {
  ...
  int get totalVotes {
    return options.fold(0, (sum, opt) => sum + opt.voterIds.length);
  }

  bool hasVoted(String userId) {
    return options.any((opt) => opt.voterIds.contains(userId));
  }

  bool hasVotedFor(String userId, String optionId) {
    final option = options.firstWhere((opt) => opt.id == optionId,
        orElse: () => PollOption(id: '', text: ''));
    return option.voterIds.contains(userId);
  }

  int getOptionVoteCount(String optionId) {
    final option = options.firstWhere((opt) => opt.id == optionId,
        orElse: () => PollOption(id: '', text: ''));
    return option.voterIds.length;
  }

  double getOptionPercentage(String optionId) {
    final count = getOptionVoteCount(optionId);
  ...
  factory Poll.fromJson(Map<String, dynamic> json) {
    return Poll(
      ...
      allowsMultiple: json['allowsMultiple'] ?? false,
    );
  }
}
...
class PollOption {
  ...
  PollOption copyWith({
    ...
    return PollOption(
      id: id ?? this.id,
      text: text ?? this.text,
      voterIds: voterIds ?? List.from(this.voterIds),
    );
  }
