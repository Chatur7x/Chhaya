...
class SearchService {
  ...
  final Map<String, Set<String>> _mediaIndex = {};
  final Map<String, MediaSearchResult> _mediaCache = {};

  Future<void> initialize(List<MeshMessage> existingMessages) async {
    await Hive.openBox<String>(_indexBox);
    for (final msg in existingMessages) {
      if (msg.type == MeshMessageType.text ||
          msg.type == MeshMessageType.audio) {
        _indexMessage(msg);
      }
    }
  ...
  void indexMediaAttachment({
    ...
    _mediaCache[messageId] = media;

    for (final token in _tokenize(fileName)) {
      _mediaIndex.putIfAbsent(token, () => {}).add(messageId);
    }

    _mediaIndex.putIfAbsent(type.name, () => {}).add(messageId);
  }

  List<MediaSearchResult> searchMedia({
    MediaType? type,
    ...
    if (type != null) {
      candidates = _mediaIndex[type.name]?.toSet() ?? {};
    } else {
      candidates = _mediaIndex.values.expand((s) => s).toSet();
    }
    ...
    for (final msgId in candidates) {
      final media = _mediaCache[msgId];
      if (media == null) continue;

      if (conversationId != null && media.conversationId != conversationId)
        continue;
      if (from != null && media.timestamp.isBefore(from)) continue;
      if (to != null && media.timestamp.isAfter(to)) continue;
  ...
  List<MediaSearchResult> searchMediaByQuery(String query, {MediaType? type}) {
    if (query.trim().isEmpty) {
      return searchMedia(type: type);
    }
    ...
    Set<String>? candidates;
    for (final token in tokens) {
      final matches = _mediaIndex[token] ?? const {};
      candidates = candidates == null
          ? matches.toSet()
          : (candidates.intersection(matches));
    }
    ...
    for (final msgId in candidates) {
      final media = _mediaCache[msgId];
      if (media == null) continue;
      if (type != null && media.type != type) continue;
      results.add(media);
    }
  ...
  Map<MediaType, List<MediaSearchResult>> getMediaGroupedByType({
    String? conversationId,
    int limit = 50,
  }) {
    final allMedia = searchMedia(conversationId: conversationId);
    final grouped = <MediaType, List<MediaSearchResult>>{
      MediaType.image: [],
      MediaType.video: [],
      MediaType.file: [],
    };

    for (final media in allMedia.take(limit)) {
      grouped[media.type]?.add(media);
    }

    return grouped;
  }

  void removeMediaFromIndex(String messageId) {
    _mediaCache.remove(messageId);
    for (final ids in _mediaIndex.values) {
      ids.remove(messageId);
    }
  }
}
